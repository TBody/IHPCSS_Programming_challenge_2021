...
PROGRAM main
    ...
    real(8), dimension(ROWS_PER_MPI_PROCESS) :: left_send_buffer, right_send_buffer
    real(8), dimension(ROWS_PER_MPI_PROCESS) :: left_recv_buffer, right_recv_buffer
        ...
        do j = 1, COLUMNS_PER_MPI_PROCESS
            do i = 1, ROWS_PER_MPI_PROCESS
                if (temperatures(i,j) == MAX_TEMPERATURE) then
                    CYCLE
                elseif (i == 0) then
                    temperatures(i,j) = (temperatures_last(i,j-1) + &
                                         temperatures_last(i,j+1) + &
                                         temperatures_last(i+1,j)) / 3.0
                elseif (i == ROWS_PER_MPI_PROCESS-1) then
                    temperatures(i,j) = (temperatures_last(i, j-1) + &
                                         temperatures_last(i, j+1) + &
                                         temperatures_last(i-1, j)) / 3.0
                else
